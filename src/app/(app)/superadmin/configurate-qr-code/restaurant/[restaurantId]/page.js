...
export default function ConfigQRPage() {
    ...
    const { data: qrCodes, isLoading: isLoadingQrCodes } = useFindAllQrCodesByRestaurantId(restaurantId)
    ...
    const items = restaurant ? [
        ...
        ...Array.from({ length: getTableCountByPlan(restaurant.owner.user_plan) }, (_, i) => ({
            name: `Table ${i + 1}`,
            id: `table-${i + 1}`,
            qrType: 'table'
        }))
    ...
    return (
        <div className="space-y-4">
            ...
            {!isLoading && !isLoadingQrCodes && items.map((item) => (
                <div key={item.id} className="flex justify-between items-center p-4 border rounded-lg">
                    ...
                    >
                        {isItemAssigned(item.name) ? 'Associé' : 'Associer le QR'}
                    </Button>
                </div>
            ))}
