            ...
            const content = (
                <div key={day} className="flex justify-between py-2">
                    ...
                    <div className="flex flex-col items-end">
                        {(!slots || slots.length === 0 || slots.every(slot => slot.closed)) ? (
                            <span>Fermé</span>
                        ) : (
                            slots.map((slot, idx) => {
                                ...
                                return (
                                    <span key={idx}>
                                        {idx === 0 ? 'Midi' : 'Soir'} : {slot.start} - {slot.end}
                                    </span>
                                )
                            })
                        )}
    ...
    return (
        <div className="min-h-screen bg-white text-slate-900">
            <div className="relative h-72">
                ...
                <Image
                    src={website.presentation_image?.url || '/images/default-restaurant.jpg'}
                    alt="Présentation"
                    fill
                ...
                {website.google_info?.url && (
                    <section className="bg-slate-900 text-white p-4 rounded-lg">
                        <div className="flex justify-between items-center">
                            <div>
                                <p className="font-medium">Je profite de -5% de réduction</p>
                                <p className="text-sm text-gray-300">
                                    Soutenez notre restaurant en laissant un avis Google
                                </p>
                            </div>
                            <div className="flex">
                                {[1, 2, 3, 4, 5].map((star) => (
