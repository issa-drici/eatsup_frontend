...
export const useCreateMenuFromAIGenerator = ({
    restaurantId,
    menuId,
    onSuccess,
}) => {
    ...
    const createMenu = async menuData => {
        ...
        try {
            const res = await axios.post(
                `/api/restaurant/${restaurantId}/menu/${menuId}/ai-menu-generator/create`,
                menuData,
            )
            if (res.data?.job_id) {
        ...
        pollingRef.current = setInterval(async () => {
            try {
                const res = await axios.get(
                    `/api/ai-menu-generator/status/${jobId}`,
                )
                ...
                if (statusPayload === 'completed') {
                    ...
                    setStatus('success')
                    setStatusMessage(
                        res.data?.data?.message || 'Menu généré avec succès !',
                    )
                    setIsPublishing(false)
                    ...
                    setStatus('processing')
                    setStatusMessage(
                        res.data?.data?.message || 'Génération en cours...',
                    )
                }
            } catch (e) {
                clearInterval(pollingRef.current)
